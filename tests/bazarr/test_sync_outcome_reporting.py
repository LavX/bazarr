...
def _run_raising(tmp_path, error, times=1):
...
def test_a_rejection_costs_the_engine_a_strike(tmp_path):
    """Bazarr overruled an engine that claimed success: it is confidently
    producing an answer this file cannot use, which is worth remembering."""
    from subtitles.tools.subsync_engines import SyncResultRejectedError

    store, subtitle = _run_raising(tmp_path, SyncResultRejectedError)

    assert store.failure_count(str(subtitle), 'alass') == 1


def test_a_decline_costs_the_engine_nothing(tmp_path):
    """The engine ran correctly and said this file is one it cannot align.

    autosubsync's quality check does that routinely on hard audio: sparse
    dialogue, heavy score, a language its model handles poorly. Counting it
    would quarantine the engine for that subtitle after three attempts, so it
    would be skipped even after the reference or the sync settings change.
    """
    from subtitles.tools.subsync_engines import SyncEngineDeclinedError

    store, subtitle = _run_raising(tmp_path, SyncEngineDeclinedError, times=5)

    assert store.failure_count(str(subtitle), 'alass') == 0
    assert not store.should_skip(str(subtitle), 'alass')
