...
def _guarantee_compat_secrets():
    ...
    for name, value in _defaults.items():
        ...
        try:
            ...
            pass
    # Per-test isolation for the Distribution Hub tables: some tests call
    # compat.register() (which seeds a legacy key, sometimes from an
    # autogenerated token) without the compat_db fixture's cleanup. Wipe the
    # key/usage rows before each test so a leftover is_legacy row can't poison
    # the next test's auth resolution. Best-effort: no-op if the table is absent.
    try:
        from app.database import (database, delete, TableCompatApiKeys,
                                  TableCompatUsage)
