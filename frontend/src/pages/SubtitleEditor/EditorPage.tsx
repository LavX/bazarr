...
export default function EditorPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useSubtitleContent(mediaType, mediaId ? Number(mediaId) : undefined, language);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (age < 86400000 && saved.cueCount > 0) { // Less than 24 hours old
  ...
  const handleRecoverDraft = useCallback(() => {
    ...
    try {
      const result = getParser(recoveryAvailable.format as SubtitleFormat).parse(recoveryAvailable.content);
  ...
  const [referenceCueList, setReferenceCueList] = useState<Array<{ startMs: number; endMs: number; text: string }>>([]);
  const [referenceLanguage, setReferenceLanguage] = useState<string | undefined>();
  ...
  const totalDurationMs = useMemo(
    () => docState.cues.reduce((sum, c) => sum + Math.max(0, c.endMs - c.startMs), 0),
    [docState.cues],
  );
  ...
  const ghostCues = useMemo(() => detectGaps(docState.cues), [docState.cues]);
  ...
  const handleSave = useCallback(() => {
    ...
    if (uncommitted !== null && uncommitted !== undefined && selectedIndex >= 0 && selectedIndex < docState.cues.length) {
      dispatch({ type: "APPLY_OP", op: createEditText(selectedIndex, uncommitted) });
    }
    ...
    const cuesForSave = uncommitted !== null && uncommitted !== undefined
      ? docState.cues.map((c, i) => i === selectedIndex ? { ...c, text: uncommitted } : c)
      : docState.cues;
        ...
        },
        {
          onSuccess: () => {
            ...
            queryClient.invalidateQueries({
              queryKey: [QueryKeys.Subtitles, "content", mediaType, Number(mediaId), language],
            });
          ...
          onError: (err) => {
            ...
            showNotification({ title: "Error", message: msg, color: "red", autoClose: 5000 });
          },
        },
      );
    } else {
              ...
              saveMutation.mutate(
                ...
                },
                {
                  onSuccess: (result) => {
                    ...
                    showNotification({ message: "Saved (overwrite)", color: "yellow", autoClose: 2000 });
                  },
                  onError: () => {
                    showNotification({ title: "Save Error", message: "Save failed even after retry.", color: "red", autoClose: 5000 });
                  },
                },
              );
              return;
            }
            const msg = status === 507
              ? "Disk full. Cannot save."
              : `Save failed: ${(err as Error).message}`;
            showNotification({ title: "Save Error", message: msg, color: "red", autoClose: 5000 });
          },
        },
      );
    }
  ...
  const handleSaveAs = useCallback((targetLang: string) => {
      ...
      },
      {
        ...
        onError: (err) => {
          ...
          if (status === 409) {
            ...
            showNotification({ title: "Error", message: `Save As failed: ${(err as Error).message}`, color: "red", autoClose: 5000 });
          }
        },
      },
    );
    ...
    saveMutation.mutate(
      ...
      },
      {
        onSuccess: (result) => {
          ...
          showNotification({ message: `Overwritten ${lang} subtitle`, color: "green", autoClose: 2000 });
          navigate(
        ...
        onError: (err) => {
          setOverwriteConfirm(null);
          showNotification({ title: "Error", message: `Overwrite failed: ${(err as Error).message}`, color: "red", autoClose: 5000 });
        },
      },
    );
  ...
  const handleDownload = useCallback(() => {
    ...
    a.href = url;
    const title = data?.mediaTitle?.replace(/[/\\?%*:|"<>]/g, "_") ?? "subtitle";
      ...
      switch (action) {
        ...
        case "delete": {
          ...
          if (indices.length > 0) {
            ...
            setSelectedIndex(Math.min(minIdx, docState.cues.length - indices.length - 1));
        ...
        case "split": {
          if (selectedIndex >= 0 && selectedIndex < docState.cues.length) {
            ...
            const splitAt = cursorPosRef.current > 0 && cursorPosRef.current < cue.text.length
              ? cursorPosRef.current
              : Math.floor(cue.text.length / 2);
            dispatch({ type: "APPLY_OP", op: createSplitCue(selectedIndex, splitAt) });
          }
          break;
        }
        case "merge": {
  ...
  const handleWaveformTimingChange = useCallback(
    (index: number, startMs: number, endMs: number) => {
      dispatch({ type: "APPLY_OP", op: createEditTiming(index, startMs, endMs) });
    },
    [],
  );
  ...
  const handleLoadReference = useCallback(async (lang: string) => {
    if (!mediaType || !mediaId) return;
    try {
      const result = await api.subtitles.getContent(mediaType, Number(mediaId), lang);
      if (result.data.exists === false || !result.data.content) {
        showNotification({ title: "No content", message: `No subtitle found for ${lang}`, color: "yellow" });
        return;
      }
      const parsed = getParser(result.data.format as SubtitleFormat).parse(result.data.content);
      setReferenceCueList(parsed.cues.map((c) => ({ startMs: c.startMs, endMs: c.endMs, text: c.text })));
      setReferenceLanguage(lang);
      showNotification({ message: `Loaded ${parsed.cues.length} reference cues (${lang})`, color: "teal", autoClose: 2000 });
    } catch {
      showNotification({ title: "Error", message: "Failed to load reference subtitle", color: "red" });
    }
  }, [mediaType, mediaId]);
  ...
  const handleRefFileSelected = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => {
      ...
      try {
        ...
        showNotification({ title: "Parse error", message: "Could not parse reference file", color: "red" });
      }
    };
    reader.readAsText(file);
    e.target.value = "";
  ...
  const nudgeTiming = useCallback(
    (mode: "start" | "end" | "both", direction: number, large: boolean) => {
      ...
      if (mode === "start" || mode === "both") newStart = Math.max(0, newStart + step);
      if (mode === "end" || mode === "both") newEnd = Math.max(0, newEnd + step);
      if (newEnd < newStart) newEnd = newStart; // prevent negative duration
      dispatch({ type: "APPLY_OP", op: createEditTiming(selectedIndex, newStart, newEnd) });
    },
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      // Ctrl+Shift+T: translate panel
      if (e.key === "T" && ctrl && e.shiftKey) {
        e.preventDefault();
        setTranslateOpen((v) => !v);
        return;
      }
      ...
      if (e.key === "ArrowDown" && e.altKey) {
        e.preventDefault();
        setSelectedIndex((prev) => Math.min(prev + 1, docState.cues.length - 1));
        return;
      }
    ...
    >
      ...
      {recoveryAvailable && (
        <Group
          ...
          px="md"
          py={6}
          style={{ background: "rgba(251, 191, 36, 0.1)", borderBottom: "1px solid rgba(251, 191, 36, 0.2)", flexShrink: 0 }}
        >
          <Text size="sm" c="#fbbf24">
            Unsaved draft found ({recoveryAvailable.cueCount} cues, {new Date(recoveryAvailable.timestamp).toLocaleTimeString()})
          </Text>
          <Button size="xs" variant="light" color="yellow" onClick={handleRecoverDraft}>
            Recover
          </Button>
          <Button size="xs" variant="subtle" color="gray" onClick={handleDismissRecovery}>
            Dismiss
          </Button>
        </Group>
      )}
      ...
      >
          ...
          {docState.cues.length === 0 ? (
            ...
            >
              ...
              <Text size="sm" c="dimmed" ta="center" maw={400}>
                Start by adding a cue manually, uploading an existing subtitle file,
                or pasting subtitle content.
              </Text>
              <Group gap="md">
                <Button
                  variant="light"
            ...
            <>
              <EditableCueTable
                ...
                visibleCueIds={bookmarkFilterActive && bookmarkedIds.size > 0 ? bookmarkedIds : undefined}
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <button
                      ...
                      style={{ background: "none", border: "none", color: "var(--bz-text-tertiary)", cursor: "pointer", fontSize: 14 }}
                    >
                      x
                    </button>
                  </div>
                  ...
                  {referenceCueList.length > 0 && (
                    <div style={{ fontSize: 11, color: "var(--bz-text-tertiary)" }}>
                      ...
                      <button
                        type="button"
                        onClick={() => { setReferenceCueList([]); setReferenceLanguage(undefined); }}
                        style={{
                          background: "none",
                          border: "none",
        ...
        >
          <VideoPreview
            ...
            seekId={seekCounter}
            currentSubtitleText={
              selectedCue && playbackTimeMs >= selectedCue.startMs && playbackTimeMs <= selectedCue.endMs
                ? selectedCue.text : undefined
            }
            onTimeUpdate={(ms) => {
          ...
          <DetailPane
            ...
            referenceText={referenceCueList.length > 0 ? (selectedRefText ?? "") : undefined}
      ...
      >
        <Stack gap="md">
          <Text size="sm">
            A subtitle file for <strong>{overwriteConfirm?.lang}</strong> already exists.
          ...
          <Group justify="flex-end" gap="sm">
            ...
            <Button color="red" loading={saveMutation.isPending} onClick={handleOverwrite}>
              Overwrite
            </Button>
          </Group>
        </Stack>
