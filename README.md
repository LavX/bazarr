...
| **Multi-engine subtitle sync** | Single engine | Multiple sync engines with a side-by-side output comparison before you keep a result. The configured maximum offset is an acceptance threshold for engines that report their offset (ffsubsync; autosubsync and alass report only success or failure), and a keep-all run where some engines failed reports those failures instead of an unqualified success. |
...
- **Acceptance threshold**: the configured maximum offset is enforced as an acceptance threshold for engines that report their offset (ffsubsync), so a result outside your tolerance is rejected rather than written; autosubsync and alass report only success or failure and cannot be held to it
- **Explicit failures**: editor sync fails clearly when no output is produced instead of silently keeping the original, and a keep-all mass sync where some engines failed reports those failures rather than an unqualified success (overwrite mode stops at the first success, so the engines behind it never run)
