...
On startup, Bazarr+ now stages the official-catalog versions of your enabled built-ins (#183, #184). A new `autoinstall_enabled_builtins()` runs before activate and register in `bazarr/init.py`, with a forced catalog refresh and a guarded state load that never raises.
